...
import { testList, categoryFilters, testQuestions } from "@/data/seed-data";
...
export default function TestsPage() {
  ...
  const sorted = [...testList].sort((a, b) => (b.recommended ? 1 : 0) - (a.recommended ? 1 : 0));

  const filtered = activeCategory === "all"
    ? sorted
    : sorted.filter((t) => t.category === activeCategory);
    ...
    <div className="space-y-5 animate-reveal-up">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((test) => {
            const hasQuestions = !!testQuestions[test.id];
            return (
              <Card
                key={test.id}
